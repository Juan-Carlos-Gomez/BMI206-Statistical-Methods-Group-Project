...
# # 87 genes from scent peaks are not in gene_name of genes
...
mtx_path <- file.path(base, dataset, "matrix.mtx")
features_path <- file.path(base, dataset, "features.tsv")
barcodes_path <- file.path(base, dataset, "barcodes.tsv")

matrix <- ReadMtx(mtx = mtx_path, features = features_path, cells = barcodes_path)
seurat_pbmc <- CreateSeuratObject(counts = matrix)
```
...
```{r}
bootstrap_score <- function(delta_pc, n_boot = 1000) {
  set.seed(1)
...
dataset <- "Arthritis"
arthritis_pc_scent <- readRDS(file = file.path(base, dataset, "gene_phastcons_scent.rds"))
arthritis_pc_atac  <- readRDS(file = file.path(base, dataset, "gene_phastcons_atac.rds"))

delta_pc_exon <- gene_results_exon$mean_exon - gene_results_noncoding$mean_cis[match(gene_results_exon$gene_id, gene_results_noncoding$gene_id)]
bootstrap_exon <- bootstrap_score(delta_pc_exon, n_boot = 1000)

# arthritis
arthritis_delta_pc_scent <- gene_results_scent$mean_scent - gene_results_noncoding$mean_cis[match(gene_results_scent$gene_name, gene_results_noncoding$gene_name)]
arthritis_bootstrap_scent <- bootstrap_score(delta_pc_scent, n_boot = 1000)
arthritis_delta_pc_atac <- gene_results_atac$mean_scent - gene_results_noncoding$mean_cis[match(gene_results_atac$gene_name, gene_results_noncoding$gene_name)]
arthritis_bootstrap_atac <- bootstrap_score(delta_pc_atac, n_boot = 1000)
...
bootstrap_results <- data.frame(
  Dataset = factor(c("Exon", "Arthritis", "Arthritis"), levels = c("Exon", "Arthritis")),
  Category = factor(c(NA, "SCENT", "ATAC"), levels = c(NA, "SCENT", "ATAC")),
  Mean = c(bootstrap_exon$mean, arthritis_bootstrap_scent$mean, arthritis_bootstrap_atac$mean),
  CI_Lower = c(bootstrap_exon$ci_lower, arthritis_bootstrap_scent$ci_lower, arthritis_bootstrap_atac$ci_lower),
  CI_Upper = c(bootstrap_exon$ci_upper, arthritis_bootstrap_scent$ci_upper, arthritis_bootstrap_atac$ci_upper)
)

# Create the dot plot
ggplot(bootstrap_results, aes(x = Dataset, y = Mean, color = Category)) +
  geom_point(size = 3, position = position_dodge(width = 0.5)) +
  geom_errorbar(aes(ymin = CI_Lower, ymax = CI_Upper), width = 0.2, position = position_dodge(width = 0.5)) +
  labs(title = "Bootstrap Results by Dataset", x = "Dataset", y = "Mean Delta PhastCons Score") +
  theme_minimal() +
  ylim(-0.2, 0.4) +
