...
# bootstrapping
```{r}
bootstrap_score <- function(delta_pc, n_boot = 10) {
  set.seed(1)
...
for (dataset in datasets) {
  ...
  scent_genes[[dataset]] <- gene_pc_scent$gene_name
...
# gene_pc_exon <- gene_pc_exon %>% filter(gene_name %in% scent_genes[["Arthritis"]])
# gene_pc_exon <- gene_pc_exon %>% filter(gene_name %in% scent_genes[["PBMC"]])
# gene_pc_exon <- gene_pc_exon %>% filter(gene_name %in% scent_genes[["NeurIPS"]])
...
p <- ggplot(bootstrap_results, aes(x = Dataset, y = Mean, color = Category)) +
  ...
  labs(y = "∆phastCons score from all cis-regions") +
  theme_minimal() +
